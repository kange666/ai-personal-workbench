...
<template>
  <div class="task-row" @dblclick="emit('edit', task)">
    <button v-if="task.status === 'draft'" class="task-confirm" title="确认后进入正式任务" @click.stop="emit('confirm', task.id)">确认</button><button v-else class="task-check" :class="{ checked: task.status === 'done' }" @click.stop="emit('toggle', task.id)">{{ task.status === "done" ? "✓" : "" }}</button>
