...
<template>
  <div v-if="open" class="activity-backdrop" @click.self="emit('close')"><aside class="activity-drawer panel worktime-drawer"><header><div><h2>{{ title || `${startDate} 工时明细` }}</h2><p>自动工时为估算，手工记录优先</p></div><button class="icon-button" @click="emit('close')">×</button></header>
    <div v-if="error" class="scan-message error">{{ error }}</div>
    <div class="worktime-summary"><div><small>有效工时</small><b>{{ formatMinutes(summary.totalMinutes) }}</b><span>{{ summary.hasManualCorrections ? '含手工修正' : '全部为估算' }}</span></div><div><small>原始估算</small><b>{{ formatMinutes(summary.estimatedMinutes) }}</b></div><div><small>手工记录</small><b>{{ formatMinutes(summary.manualMinutes) }}</b></div></div>
