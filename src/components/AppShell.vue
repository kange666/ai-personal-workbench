...
function handleKeydown(event: KeyboardEvent) { if ((event.ctrlKey || event.metaKey) && event.key.toLowerCase() === "k") { event.preventDefault(); openSearch(); } }
async function refreshLocalData() { refreshing.value = true; await store.hydrate(); refreshing.value = false; }
onMounted(() => { statusTimer = window.setInterval(store.refreshTaskStatuses, 60 * 60 * 1000); window.addEventListener("keydown", handleKeydown); window.addEventListener("open-workbench-search", openSearch); });
onBeforeUnmount(() => { window.clearInterval(statusTimer); window.removeEventListener("keydown", handleKeydown); window.removeEventListener("open-workbench-search", openSearch); });
</script>
...
<template>
  <div class="app-shell">
    ...
    <header class="app-topbar">
      ...
      <span class="top-date">{{ dateText }}</span><button class="icon-button" title="刷新本地任务数据" :disabled="refreshing" @click="refreshLocalData">{{ refreshing ? '↻' : '◌' }}</button><ThemeSwitch /><RouterLink class="avatar" to="/settings" title="打开本地用户设置">L</RouterLink>
    </header>
