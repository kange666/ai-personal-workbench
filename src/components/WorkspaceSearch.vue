...
<template>
  <div v-if="open" class="search-backdrop" @click.self="emit('close')" @keydown.esc="emit('close')">
    <section class="workspace-search panel">
      ...
      <label><span>⌕</span><input ref="input" v-model="query" placeholder="搜索任务、对话、报告、知识、内容和接口" @keyup.enter="search"><button class="button primary" :disabled="loading" @click="search">{{ loading ? '搜索中…' : '搜索' }}</button></label>
