...
const projects = ref<TestProject[]>(isTauriRuntime() ? [] : [{ path:"F:/TB-project/scaq-client", name:"生产安全pc", projectKind:"vue", caseCount:1, pageCount:2, capabilities:{mock:true,realApi:true,sourceStyle:true,browserStyle:true}, warnings:[] }]);
...
.testing-tabs button{padding:9px 16px;border:0;border-radius:8px;background:transparent;color:var(--muted);cursor:pointer}.testing-tabs button.active{background:var(--primary-fill);color:#fff}
...
/* 对照证据较长时仍保留关闭入口，标题列可收缩以容纳长功能名。 */
.parity-detail>header{position:sticky;top:-24px;z-index:2;display:flex;align-items:flex-start;justify-content:space-between;gap:16px;padding:12px 0;background:var(--surface)}
.parity-detail>header>div{min-width:0;overflow-wrap:anywhere}.parity-detail>header h2{margin:4px 0}.parity-detail>header p{margin:0}.parity-detail>header>.icon-button{flex-shrink:0}
