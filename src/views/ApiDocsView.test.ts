...
vi.mock("../services/backend", () => ({
  ...
  listApiEndpoints: async () => [
    { id:"ep-1",sourceId:"source-1",operationId:"getUser",method:"GET",path:"/users/{id}",title:"查询用户",description:"按编号查询",tags:["用户"],deprecated:false,favorite:false,updatedAt:"" },
    { id:"ep-2",sourceId:"source-1",operationId:"deleteUser",method:"DELETE",path:"/users/{id}",title:"删除用户",description:"",tags:["用户/管理"],deprecated:true,favorite:true,updatedAt:"" },
  ],
  ...
  setApiEndpointFavorite: mocks.favorite,
  ...
  saveApiTestConfig: vi.fn(), clearApiTestToken: vi.fn(),
...
describe("ApiDocsView", () => {
  ...
  it("按项目展示接口、筛选并复制后端生成的 Markdown", async () => {
    ...
    expect(wrapper.findAll("option").some(option=>option.text().includes("全部状态"))).toBe(false);
    ...
    await wrapper.findAll("button").find(button=>button.text().includes("收藏"))!.trigger("click");
    ...
    expect(mocks.favorite).toHaveBeenCalledWith("ep-1",true);
  ...
  it("项目代码模板可独立配置并保存", async () => {
    const router=createRouter({history:createMemoryHistory(),routes:[{path:"/api-docs",component:ApiDocsView},{path:"/settings",component:{template:"<div />"}}]});
    await router.push("/api-docs");await router.isReady();
    const wrapper=mount(ApiDocsView,{global:{plugins:[router]}});await flushPromises();
    await wrapper.findAll("button").find(button=>button.text()==="代码模板")!.trigger("click");await flushPromises();
    await wrapper.get(".api-code-template-editor select").setValue("axios");
    await wrapper.get('.api-code-template-editor input[placeholder="例如 _ 或 api"]').setValue("api");
    await wrapper.findAll("button").find(button=>button.text()==="保存模板")!.trigger("click");await flushPromises();
