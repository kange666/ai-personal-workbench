    ...
    <section class="calendar-layout" :class="`mode-${viewMode}`">
      <article v-if="viewMode !== 'gantt'" class="panel calendar-panel">
        ...
        <div class="month-grid">
          <div v-for="day in calendarDays" :key="day.iso" class="calendar-day" :class="{ muted:!day.current, today:day.today, active: selectedDate === day.iso }" @click="openDay(day.iso)">
            <b>{{ day.value }}</b>
            <button class="lunar-day-label" title="查看该日黄历" @click.stop="selectedAlmanacDate=day.iso">{{ lunarDayLabel(day.iso) }}</button>
            <button v-for="task in tasksForDate(day.iso).slice(0,2)" :key="task.id" class="calendar-task" :class="task.status" :title="`任务：${task.title}`" @click.stop="selectedTask = task">任务 · {{ task.title }}</button>
            <div class="calendar-day-meta">
              <button v-if="activityByDate.get(day.iso)?.workMinutes" class="calendar-worktime" :title="`工时：${minutesText(activityByDate.get(day.iso)?.workMinutes || 0)}（${activityByDate.get(day.iso)?.manualWorkMinutes ? '已修正' : '估算'}）`" @click.stop="selectedTimeDate=day.iso">工时 {{ minutesText(activityByDate.get(day.iso)?.workMinutes || 0) }}</button>
              <button v-if="activityByDate.get(day.iso)?.conversationCount" class="calendar-activity" :title="`Codex：${activityByDate.get(day.iso)?.conversationCount} 次对话，${(activityByDate.get(day.iso)?.totalTokens || 0).toLocaleString()} Token`" @click.stop="openDay(day.iso)">Codex {{ activityByDate.get(day.iso)?.conversationCount }}</button>
              <button v-if="activityByDate.get(day.iso)?.testRuns" class="calendar-test" :title="`测试：通过 ${activityByDate.get(day.iso)?.testsPassed} / 执行 ${activityByDate.get(day.iso)?.testRuns}`" @click.stop="router.push(`/testing?date=${day.iso}`)">测试 {{ activityByDate.get(day.iso)?.testsPassed }}/{{ activityByDate.get(day.iso)?.testRuns }}</button>
              <button v-if="activityByDate.get(day.iso)?.contentIdeaCount" class="calendar-content" :title="`内容选题：${activityByDate.get(day.iso)?.contentIdeaCount} 个`" @click.stop="router.push(`/content?date=${day.iso}`)">选题 {{ activityByDate.get(day.iso)?.contentIdeaCount }}</button>
              <button v-if="activityByDate.get(day.iso)?.dailyReportId" class="calendar-report" title="日报已生成，点击打开" @click.stop="openReport(activityByDate.get(day.iso)?.dailyReportId)">日报</button>
            </div>
            <small v-if="tasksForDate(day.iso).length > 2">还有 {{ tasksForDate(day.iso).length - 2 }} 项任务</small>
          </div>
        </div>
      </article>
