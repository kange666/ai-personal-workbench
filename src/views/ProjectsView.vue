...
async function startProject(item: RepositoryAsset) {
  if (!item.startCommand || startingPath.value) return;
  startingPath.value = item.path; error.value = ""; message.value = "";
  try {
    const result = await startRepositoryProject(item.path);
    message.value = `${result.message} 启动命令：${result.command}`;
  } catch (cause) { error.value = String(cause); }
...
<template>
  <div class="view projects-view">
    ...
    <section class="panel asset-workspace">
      <div class="asset-toolbar"><label>⌕<input v-model="query" placeholder="搜索项目、路径、用途或技术栈"></label><select v-model="categoryFilter"><option>全部分类</option><option v-for="item in categories" :key="item">{{ item }}</option></select><select v-model="health"><option>全部状态</option><option>健康</option><option>警告</option><option>失败</option><option>未验证</option></select><select v-model="changes"><option>全部工作区</option><option>有未提交修改</option><option>工作区干净</option></select><span>{{ filtered.length }} / {{ visibleAssets.length }}</span></div>
      <div class="asset-table-wrap"><table class="asset-table"><thead><tr><th class="pin-column">置顶</th><th>项目</th><th>用途与分类</th><th>技术栈</th><th>关联数据</th><th>健康</th><th>工作区</th><th>操作</th></tr></thead><tbody><tr v-for="item in filtered" :key="item.path" :class="{ pinned: item.isPinned }" @click="openAsset(item)"><td><button class="pin-button" :class="{ active: item.isPinned }" :title="item.isPinned ? '取消置顶' : '置顶项目'" @click.stop="togglePin(item)">{{ item.isPinned ? "★" : "☆" }}</button></td><td><b>{{ item.name }}</b><small>{{ item.path }}</small></td><td><button class="category-button" title="点击修改分类" @click.stop="openCategoryEditor(item)">{{ item.category }}</button><small>{{ item.purpose || "AI 推断待确认" }}</small></td><td>{{ item.technologyStack || "待确认" }}</td><td><span>{{ item.conversationCount }} 个 Codex 任务</span><small>{{ item.commitCount }} 条最近提交</small></td><td><i class="health-pill" :class="healthClass(item.healthLevel)">{{ item.healthLevel }}</i></td><td><i v-if="item.hasUncommittedChanges" class="dirty-pill">有修改</i><span v-else class="clean-text">干净</span></td><td><div class="asset-row-actions"><button class="row-launch-button" :disabled="!item.startCommand || Boolean(startingPath)" :title="item.startCommand || '请先在项目详情中配置启动命令'" @click.stop="startProject(item)">{{ startingPath === item.path ? '启动中…' : '▶ 启动' }}</button><button class="text-button muted-action" @click.stop="toggleHidden(item, true)">隐藏</button><button class="text-button" @click.stop="openAsset(item, 'git')">Git →</button></div></td></tr></tbody></table><div v-if="!filtered.length" class="empty-state"><b>没有符合条件的项目</b><p>请调整分类或其他筛选条件。</p></div></div>
    </section>
