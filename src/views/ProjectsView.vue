        ...
        <section class="commit-plan">
          ...
          <div v-if="details.commitPlan && gitStatus?.hasUncommittedChanges" class="commit-plan-summary">
            ...
            <article v-for="group in details.commitPlan.groups" :key="group.id">
              ...
              <textarea v-model="commitMessages[group.id]" rows="4" :disabled="group.status === 'committed'" aria-label="提交信息和修改明细"></textarea>
              <small class="commit-message-help">第一行为提交标题，空一行后每项修改使用“- ”开头；这些内容仍会合成一次提交。</small>
