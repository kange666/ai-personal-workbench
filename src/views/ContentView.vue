...
<template>
  <div class="view content-view">
    <header class="page-header"><div><h1>内容工坊</h1><p>{{ contentType === 'reasoning' ? '每日 5 个逻辑选题' : '每日 5 个科技选题' }}</p></div><div><input v-model="selectedDate" class="button secondary content-date" type="date"><button class="button primary" :disabled="loading" @click="regenerate">{{ loading ? '生成中…' : '✦ 重新生成 5 条' }}</button></div></header>
    <nav class="content-channel-switch panel"><button :class="{ active:contentType === 'tech' }" @click="switchContentType('tech')"><b>未来科技探索</b></button><button :class="{ active:contentType === 'reasoning' }" @click="switchContentType('reasoning')"><b>逻辑思维案例</b><small>5 题</small></button></nav>
    <p v-if="message" class="scan-message">{{ message }}</p><p v-if="error" class="scan-message error">{{ error }}</p>
    <section class="content-summary panel"><div><b>{{ ideas.length }}</b><span>今日候选</span></div><div><b>{{ selectedCount }}</b><span>已选择</span></div><div><b>{{ ideas.filter(item => item.status === 'rejected').length }}</b><span>已淘汰</span></div><details><summary>内容规则</summary><span>{{ contentType === 'reasoning' ? '每天覆盖不同思维类型；每题先验证唯一答案，再提供完整解法。' : '不伪装开箱或亲身体验；趋势与推测会明确表达。' }}</span></details></section>
    <section class="content-layout">
      <aside class="panel content-list"><header><b>{{ selectedDate }} 候选标题</b></header><button v-for="(item,index) in ideas" :key="item.id" :class="[item.status,{ active:selected?.id === item.id }]" @click="choose(item)"><i>{{ index + 1 }}</i><span><small>{{ item.category }} · {{ item.source === 'deepseek' ? 'AI 生成' : '本地生成' }}</small><b>{{ item.title }}</b><em>{{ statusLabel[item.status] }}</em></span></button><p v-if="!ideas.length && !loading" class="panel-empty">当天还没有候选内容。</p></aside>
      <main v-if="selected" class="panel content-detail">
        ...
        <section v-if="selected.status === 'selected'" class="codex-job-panel" :class="activeJob?.status || 'ready'">
          ...
          <div v-if="activeJob" class="codex-progress"><div><span>{{ activeJob.progressMessage || jobStatusLabel[activeJob.status] }}</span><b>{{ activeJob.progressPercent }}%</b></div><div class="codex-progress-track"><i :style="{width:`${activeJob.progressPercent}%`}"></i></div><small>当前阶段：{{ jobStageLabel[activeJob.currentStage] }}</small></div>
