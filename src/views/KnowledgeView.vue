...
<template>
  <div class="view">
    <header class="page-header"><div><h1>知识库</h1><p>从历史工作中提炼可复用方案</p></div><div><button class="button secondary" @click="resetForm()">＋ 手动补充</button><button class="button primary" :disabled="syncing" @click="autoSummarize">{{ syncing ? '整理中…' : '✦ 重新提炼知识' }}</button></div></header>
    <div v-if="error || message" class="scan-message" :class="{ error: Boolean(error) }">{{ error || message }}</div>
    <section class="knowledge-search"><div><b>◇ 搜索可直接复用的做法</b><p>基于已确认知识回答</p></div><select v-model="activeProject" class="button secondary"><option v-for="item in projects" :key="item">{{ item }}</option></select><label>⌕<input v-model="query" placeholder="例如：页面开发时字典怎么接入？" @keyup.enter="ask" /><kbd @click="ask">Enter</kbd></label></section>
