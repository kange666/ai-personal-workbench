...
const demos: KnowledgeItem[] = [
  { id: "demo-k1", kind: "decision", title: "本地桌面工作台应采用什么数据存储方案？", content: "适用场景：单用户桌面应用需要离线保存任务、报告和知识。\n核心结论：使用 SQLite，应用无需额外启动数据库服务。\n实施方法：\n1. 将数据库文件放在应用数据目录。\n2. 通过版本号执行增量迁移。\n注意事项：密钥不要写入 SQLite，应保存到系统凭据库。", project: "星枢工作台", sourceType: "report", sourceId: "demo-report", tags: "可复用方法,SQLite,本地优先", confirmed: true, createdAt: now, updatedAt: now },
  { id: "demo-k2", kind: "experience", title: "Codex Token 应如何按会话和日期准确统计？", content: "适用场景：需要同时展示会话总量和每日 Token 趋势。\n核心结论：会话总量读取最后一次累计值，每日用量计算相邻事件的正向差量。\n实施方法：\n1. 按时间排序 Token 事件。\n2. 仅累加大于零的差值并按日期归组。\n验证标准：各日期用量之和不应大于会话累计总量。", project: "星枢工作台", sourceType: "conversation", sourceId: "demo-2", tags: "可复用方法,Codex,Token", confirmed: true, createdAt: now, updatedAt: now },
  { id: "demo-k3", kind: "risk", title: "自动周报生成时如何避免遗漏当天工作？", content: "适用场景：周报与日报可能在同一天定时生成。\n核心结论：周报直接读取任务、对话和 Git 原始数据，不依赖日报是否已生成。\n注意事项：若周报只汇总日报，22:00 前的当日日报不存在时会漏掉整天内容。", project: "星枢工作台", sourceType: "manual", tags: "可复用方法,报告,调度", confirmed: true, createdAt: now, updatedAt: now },
];
