  ...
  <div class="view api-docs-view">
    ...
    <section class="panel api-docs-layout">
      <main class="api-endpoint-panel">
        <header><div class="api-endpoint-heading"><label class="api-project-switch"><select :value="selectedSourceId" aria-label="切换 Apifox 项目" @change="selectSourceById(($event.target as HTMLSelectElement).value)"><option value="" disabled>请选择 Apifox 项目</option><option v-for="item in sources" :key="item.id" :value="item.id">{{ item.apifoxProjectName || item.documentTitle || `Apifox ${item.externalProjectId}` }}</option></select><small v-if="activeSource">项目 ID：{{ activeSource.externalProjectId }} · 更新于 {{ formatTime(activeSource.lastSyncedAt) }}</small><small v-else>请选择或新增 Apifox 项目</small></label></div><div v-if="activeSource" class="api-source-actions"><button class="button secondary small" :disabled="syncingSourceId===activeSource.id || !credential.configured" @click="syncOne(activeSource)">{{ syncingSourceId===activeSource.id ? "同步中…" : "↻ 同步" }}</button></div></header>
        ...
        <div class="api-endpoint-list" aria-label="接口目录树">
          ...
          <p v-if="activeSource && !filtered.length">{{ endpoints.length ? "没有符合筛选条件的接口。" : "尚无缓存，请点击同步。" }}</p><p v-if="!activeSource">请从项目名称下拉中选择或新增一个 Apifox 项目。</p>
        </div>
      </main>
...
.api-docs-layout{flex:1;min-height:0;display:grid;grid-template-columns:300px minmax(0,1fr);overflow:hidden}.api-endpoint-panel,.api-detail-panel{min-width:0;min-height:0;display:flex;flex-direction:column}.api-endpoint-panel{border-right:1px solid var(--line)}.api-endpoint-panel>header{min-height:67px;padding:11px 14px;border-bottom:1px solid var(--line);display:flex;align-items:center;justify-content:space-between;gap:10px}.api-endpoint-heading{min-width:0;display:flex!important;align-items:center;gap:9px!important}.api-project-switch{min-width:0;display:grid;gap:5px}.api-project-switch select{width:min(240px,100%);border:0;background:transparent;color:var(--text);font-size:18px;font-weight:700;line-height:1.2;outline:0;cursor:pointer;overflow:hidden;text-overflow:ellipsis}.api-project-switch small{color:var(--muted);font-size:10px;white-space:nowrap;overflow:hidden;text-overflow:ellipsis}.api-endpoint-list{min-height:0;overflow-y:auto;overflow-x:hidden;flex:1;scrollbar-gutter:stable}.api-endpoint-list>p{padding:24px 14px;color:var(--muted);line-height:1.7}
