...
- 日报、周报、月报：根据任务、Codex 对话、Git 提交、TAPD 缺陷、测试结果和轻量工时生成，按项目回答“做了什么功能”，过滤停止服务、忽略测试文件等低价值过程；报告提供来源证据链、数据截止时间，可编辑、锁定、重新生成、AI 总结及导出 Word。
...
- DeepSeek：API Key 保存在 Windows 凭据库，不写入 SQLite；用于提炼独立的报告 AI 总结和已确认知识问答。
...
- 点击“AI 总结”时，只会把当前报告正文发送给 DeepSeek，返回的精炼总结独立保存，不会替换原报告；知识问答只发送已确认知识。
